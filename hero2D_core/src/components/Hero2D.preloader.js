 ...
 */
 	...
 	Preloader.prototype.requestFile = function(filename, file, callback) {
		...
		currentQuery.open("GET", XMLH2D_currentPath + requestedFile, true);
		...
		function progress(event) {
			if(self.type[filename] == "sound") {
				self.buffers[filename] = new Audio(XMLH2D_currentPath + requestedFile);
			}
 	...
 	Preloader.prototype.$ = function(name) {
 		if(this.type[name] == "sound") {
			...
			return XMLH2D_currentPath + this.files[name];
		}
 	}